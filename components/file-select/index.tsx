...
import IconUpload from '@hankliu/icons/UploadOutlined';
...
export default class FileSelect extends React.Component<FileSelectProps, {}> {
  ...
  open = () => {
    const { accept, maxsize, maximum, multiple, onSelect } = this.props;
    selectFiles({
      accept,
      maxsize,
      maximum,
  ...
  onDrop = (event: any) => {
    ...
    // console.log(files);
    const { onSelect, accept, maxsize, multiple } = this.props;
  ...
  render() {
    const { style = {}, width, height, className, children, disabled, drawable } = this.props;
  ...
  renderInner(children: React.ReactNode) {
    ...
    const { maxsize = MAXSIZE, icon, title = locale.title, height } = this.props;
    ...
    return (
      <div className={cs({ 'select-box': true, 'fix-height': height })}>
        <div className="box-icon">{iconE}</div>
