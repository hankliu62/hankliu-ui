...
| status | 设置校验状态 | 'error' \| 'waring' | - | 4.19.0 |
| prefix | 带有前缀图标的 input | ReactNode | - |  |
| size | 控件大小。注：标准表单内的输入框大小限制为 `large` | `large` \| `middle` \| `small` \| `medium` \| `smedium` | - |  |
...
| size | `Input.Group` 中所有的 `Input` 的大小，可选 `large` `default` `small` `medium` `smedium` | string | `default` |
...
| 参数 | 说明 | 类型 | 默认值 | 版本 |
| --- | --- | --- | --- | --- |
| iconRender | 自定义切换按钮 | (open) => ReactNode | (open) => (open ? &lt;EyeOutlined /> : &lt;EyeInvisibleOutlined />) | 4.3.0 |
